...
struct Drone : public AiUnit
{
	...
	glm::vec2 getThrust() const
	{
		...
		glm::vec2 thrust_left = left.getPower() * glm::vec2(cos(angle_left), sin(angle_left));
		const float angle_right = angle + right.angle - HalfPI;
		glm::vec2 thrust_right = right.getPower() * glm::vec2(cos(angle_right), sin(angle_right));
	...
	float getTorque() const
	{
		...
		// const float left_torque = left.getPower() / thruster_offset * cross(glm::vec2(cos(angle_left), sin(angle_left)), glm::vec2(1.0f, 0.0f));
		const float left_torque = left.getPower() / thruster_offset * cross(glm::vec2(cos(angle_left), sin(angle_left)), glm::vec2(1.0f, 0.0f));
		...
		const float right_torque = right.getPower() / thruster_offset * cross(glm::vec2(cos(angle_right), sin(angle_right)), glm::vec2(-1.0f, 0.0f));
	...
	void update(float dt, bool update_smoke)
	{
		...
		const glm::vec2 gravity(0.0f, 1000.0f);
		// Integration
	...
	// {
	// 	return getAngle(glm::vec2(cos(angle), sin(angle))) / PI;
	// }

// ! i assummed that outputs will come from AI actions decisions
// ! like from aa.execute() -> std::vector<float> -> outputs vector
